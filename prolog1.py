...
def unify (srcTerm, srcEnv, destTerm, destEnv) :
    ...
    for i in range(nargs) :
        # the difference between arg and value is that arg is what is inputted, value is what the unification engine outputs
        srcArg  = srcTerm.args[i]
        destArg = destTerm.args[i]
        if srcArg <= 'Z' : srcVal = srcEnv.get(srcArg) # by being less than or equal to 'Z', we can say anything that begins with a capital letter is a variable. (Lowercase letters come after uppercase letters)
        else             : srcVal = srcArg
        if srcVal :    # constant or defined Variable in source, if it is nil, that means it's not in srcEnv 
